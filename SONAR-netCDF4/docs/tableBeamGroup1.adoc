...
|:units = "s" | |
| | |
|float blanking_interval(ping_time, beam) |M |Time between first sample in data array and the first valid values.
|:long_name = "Time between first recorded sample in data array and the first valid value " | |
|:units = "s" | |
|:valid_min = "0.0" | |
| | |
...
| | |
|float sound_speed_at_transducer(ping_time) |O |Sound speed at tx transducer depth at ping time
