...
When sub-beams from a split-aperture transducer are provided, it is also necessary to combine the sub-beams to give the power or amplitude of the echoes, as received by the whole beam. All known split-aperture systems achieve this by the simple addition of the sub-beams values, noting that the sub-beam data are typically complex power or amplitude values.
...
This arrangement has four equal quadrants (Part A, <<transducer_types>>) in the transducer and the major and minor angles are calculated thus:

==== Three equal sectors
This arrangement has three equal triangular sectors (Part B, <<transducer_types>>) and the major and minor angles are calculated thus (citenp:[bodholt2001, bjorno2017]):

==== Three equal sectors and one central sector
The arrangement has three truncated triangular sectors surrounding one central circular sector (Part C, <<transducer_types>>). The major and minor angles are calculated thus:
