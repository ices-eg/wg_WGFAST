...
When sub-beams from a split-aperture transducer are provided, it is also necessary to combine the sub-beams to give the power or amplitude of the echoes, as received by the whole beam. All known split-aperture systems achieve this by the simple addition of the sub-beam values, noting that the sub-beam data are typically complex power or amplitude values.
...
The sonar does not calculate the echo arrival angle - they are calculated from the split-aperture sub-beams, each of which are stored in the SONAR-netCDF4 files. Several types of sub-beam arrangements are supported (<<transducer_types>>) - the angle calculation methods for each type are given in the following sections. The sub-beams are stored as the _sector_ dimension in the backscatter variables in <<sonarBeammodeTable>>, in the same order as labelled in <<transducer_types>>.

.The split-aperture transducer arrangements supported by SONAR-netCDF4 (A: 4-quadrants; B: 3-sectors; C; 3+1-sectors). The Cartesian coordinates show the orientation of the sonar beam coordinate system relative to the transducer sectors (note that the _z_-axis points into the page). The numbers indicate the sub.beam labels for each type of transducer.
[[transducer_types]]
