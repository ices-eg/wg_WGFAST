...
Optionally, /Platform subgroups can be included to store data from individual instruments measuring values that are relative to the platform. If those values are relative to the environment around the platform, they must be stored in the Environment group. 
