...
When sub-beams from a split-aperture transducer are provided, it is also necessary to combine the sub-beams to give the power or amplitude of the echoes, as received by the whole beam. All known split-aperture systems achieve this by the simple addition of the sub-beams value, noting that the sub-beam data are typically complex power or amplitude values.

To allow for existing conventions on how the split-aperture angles are stored or derived (typically as phase angles between the split-aperture signals), angle scaling factors are given in variables _echoangle_major_scaling_factor_ and _echoangle_minor_scaling_factor_.

=== Angles stored in file
The calculation of the echo arrival angle is done by the sonar and the SONAR-netCDF4 file contains either physical arrival angles or phase angles. For the latter, the major (latexmath:[\theta]) and minor (latexmath:[\phi]) physical arrival angles are obtained by dividing by the major latexmath:[\eta_\theta] and minor latexmath:[\eta_\phi] angle scaling factors respectively. For consistency, when physical arrival angles are stored in the file, latexmath:[\eta_\theta] and latexmath:[\eta_\phi] should still be present and be set to 1.0.

=== Angles not stored in file
The sonar does not calculate the echo arrival angle and they must instead be calculated from the split-aperture sub-beams, each of which are hence stored in the SONAR-netCDF4 files. Several types of sub-beam arrangements are supported (<<transducer_types>>) - the angle calculation methods for each type are given in the following sections.

.The split-aperture transducer arrangements supported by SONAR-netCDF4 (A: 4-quadrants; B: 3-sectors; C; 3+1-sectors). The Cartesian coordinates show the orientation of the sonar beam coordinate system relative to the transducer sectors (note that the _z_-axis goes into the page). The numbers indicate the sub.beam labels for each type of transducer.
[[transducer_types]]
...
++++
\begin{equation}
\theta = \arctan \left( \frac{\Im((y_2+y_3)^*)}{\Re(y_1+y_4)} \right) \frac{1}{\eta_\theta}\\
\phi = \arctan \left( \frac{\Im((y_1+y_2)^*)}{\Re(y_3+y_4)}  \right) \frac{1}{\eta_\phi} 
\end{equation}
++++
where latexmath:[\arctan] is the four-quadrant inverse tangent, latexmath:[y_x] the complex signal from quadrant latexmath:[x], * indicates the complex conjugate and latexmath:[\Re] and latexmath:[\Im] the real and imaginary parts of the complex numbered signals. The angle scaling factors are latexmath:[\eta_\theta] for the major axis and latexmath:[\eta_\phi] for the minor axis.


==== Three or four sectors (type B and C)

This arrangement has either three equal sectors (<<transducer_types>>, partB) or a centre sector and three surrounding sectors. (<<transducer_types>>, part C). Type C can be treated the same as type B once the centre sector signal is added to each of the outer sector signals. The phase angles between two pairs of the three sectors are given by cite:[bodholt2001, bjorno2017]:

[stem]
++++
\begin{equation}
\omega_{31} = \arctan \left( \frac{\Im((y_3+y_1)^*)}{\Re(y_3+y_1)}   \right) \\
\omega_{32} = \arctan \left( \frac{\Im((y_3+y_2)^*)}{\Re(y_3+y_1)}   \right)
\end{equation}
++++
...
\phi = \frac{1}{\sqrt{3}\eta_\phi} (\omega_{32} - \omega_{31})\\
\end{equation}
++++
where latexmath:[\eta_\theta] is the angle scaling factor for the major axis and latexmath:[\eta_\phi] the angle scaling factor for the minor axis.
...
Split-aperture echo arrival angles are given as Cartesian angles in the beam coordinate system:
