...
Optionally, /Platform subgroups can be included to store data from individual instruments. Each platform can be made up with several instruments and especially several position and attitude sensors can be associated to the platform.
Individual position and (resp.) attitude instruments are defined individually as subgroups of the */Platform/Position* and (resp.) */Platform/Attitude* subgroups. The name of those subgroups shall match the names of their serial number or identifier as defined in <<platformGroupTable>>.
The coordinate system convention, including the definition of pitch and roll, is detailed in <<_coordinate_systems>>.
The platform, position and attitude subgroups would also provide a standard place to store unprocessed and unparsed instrument data from multiple GPS sensors, motion reference units, etc.
