...
Type 3 conversion equations are intended for data recorded by Simrad EK60, ES60, and ES70 echosounders.

=== Type 4

Type 4 conversion equations are intended for data recorded by Simrad EK80 and ES80 broadband echosounders.
...
The calculation of the echo arrival angle is done by the sonar and the SONAR-netCDF4 file contains these angles. These are stored as _electrical_angles_ and are converted by the reading software into physical angles by using an _angle_factor_ conversion:

Note that an _angle_factor_ of 1.0 implies that the angles were stored as physical, not electrical angles.
...
This arrangement has four equal quadrants in the transducer (<<transducer_types>>, part A) and the major and minor angles are calculated thus:

==== Three or four sectors (type B and C)

This arrangement has either three equal sectors (<<transducer_types>>, partB) or a centre sector and three surrounding sectors. (<<transducer_types>>, part C). Type C can be treated the same as type B once the centre sector signal is added to each of the outer sector signals. The major and minor angles are then calculated thus(citenp:[bodholt2001, bjorno2017]):

[stem]
++++
\begin{equation}
u_x = \frac{1}{\sqrt{3\Gamma}} (\theta_{31} + \theta_{32})
\end{equation}
++++
...
* The _minor_ arrival angle is given by the angle from the _z'_ axis, measured in the _x'_-_z'_ plane, and
* The _major_ arrival angle is given by the angle from the _z'_ axis measured in the _y'_-_z'_ plane.
