...
2. Open the Git extension by clicking on its icon on the right side of the screen. +
...
1. Open the Git extension and click on the image:Brackets_origin_button.png[] button. +
2. Choose _Create new remote_. +
3. You can name your new remote anything you would like, but it is a good idea to call it `upstream` to stay in the Git parlance. +
...
Note: You might also have to use the `Pull` button. I have not done this yet as no pull requests have been merged to `master` in the upstream repository.
{empty} +
...
3. Make sure the origin branch is `master` and add a name for your working branch. +
...
To open the AsciiDoc live preview pane, click on the AsciiDoc extension in the right side of the screen. The extension icon will be present with an AsciiDoc document is open. +
{empty} +
